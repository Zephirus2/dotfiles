...
class Labels:
    ...
    NORMALIZE_SCALE_DESC='Automatically scale selected islands before packing so that the average texel density is the same for every island. WARNING: if lock overlapping is enabled, then normalization takes place AFTER overlapping islands are locked. WARNING 2: islands packed with fixed scale will not be normalized'
    ...
    PACK_OP_TYPE_PACK_DESC = "Pack selected UV islands into the target UV area. 'Other' islands will be ignored. {}".format(OTHER_ISLANDS_DESC)
    PACK_OP_TYPE_PACK_TO_OTHERS_DESC = "Pack selected UV islands into the target UV area so that they do not overlap with 'Other' islands. {}".format(OTHER_ISLANDS_DESC)
    PACK_OP_TYPE_REPACK_WITH_OTHERS_DESC = "Repack selected UV islands into the target UV area together will all unselected islands which are already placed in the target area (or overlap the area by some part). Unselected islands which do not overlap the target area will be ignored. If the '{}' option is enabled (in the Addon Preferences multi panel), islands will be packed so they do not overlap with pinned islands".format(PINNED_UVS_AS_OTHERS_NAME)
